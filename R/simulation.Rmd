...
sigma.sq <- runif(fraction*n_genes, 0, 0.1)
sigma.sq <- append(sigma.sq, runif(fraction*n_genes, 0.9, max_sigma.sq))
...
beta <- runif(n_genes*fraction, log(high_range_beta[1]), log(high_range_beta[2]))
beta <- append(beta, runif((1-fraction)*n_genes, log(low_range_beta[1]), log(low_range_beta[2])))
...
load(file = "mean_var_project/simulation/poisson_3/spe_simulation_weighted_constrained_nnSVG.Rdata")
...
#load(file = "simulation_BRISC_estimation_spline.rds")
load(file = "mean_var_project/simulation/poisson_3/constrained_weights.rds")
w <- w_new
...
data.frame(x = rowData(spe_unweighted)$mean,
           y = rank_shift,
           ground_truth = rowData(spe_unweighted)$ground_truth
) |>
  ggplot() +
  geom_point(aes(x = x, y = y, color = ground_truth)) +
  labs(title = "Rank Changes simulation",
       x = "mean expression",
...
lrt_unw <- data.frame(
  y = rank(-rowData(spe_unweighted)$LR_stat),
  x = rowData(spe_unweighted)$mean,
  ground_truth = rowData(spe_unweighted)$ground_truth
) |> 
  ggplot() +
...
prop_unw <- data.frame(
  y = rank(-rowData(spe_unweighted)$prop_sv),
  x = rowData(spe_unweighted)$mean,
  ground_truth = rowData(spe_unweighted)$ground_truth
) |> 
  ggplot() +
  ...
  labs(
    x = "mean of logcounts",
    y = "rank(-prop. spatial var.)",
    title = "unweighted nnSVG"
  )

prop_w <- data.frame(
  y = rank(-1*unlist(lapply(weighted_nnSVG_list, function (x) x[c('weighted_prop_sv')]))),
  x = weighted_mean,
  ground_truth = rowData(spe_unweighted)$ground_truth
) |> 
  ggplot() +
...
rank_sigma_unw <- data.frame(
  y = rank(-rowData(spe_unweighted)$sigma.sq),
  x = rowData(spe_unweighted)$mean,
  ground_truth = rowData(spe_unweighted)$ground_truth
) |> 
  ggplot() +
...
rank_sigma_w <- data.frame(
  y = rank(-1*unlist(lapply(weighted_nnSVG_list, function (x) x[c('weighted_sigma.sq')]))),
  x = weighted_mean,
  ground_truth = rowData(spe_unweighted)$ground_truth
) |> 
  ggplot() +
...
sigma_w <- data.frame(
  y = unlist(lapply(weighted_nnSVG_list, function (x) x[c('weighted_sigma.sq')])),
  x = weighted_mean,
  ground_truth = rowData(spe_unweighted)$ground_truth
) |> 
  ggplot() +
...
wrap_plots(sigma_unw, sigma_w, guides = "collect", nrow = 1)
...
mean_data <- data.frame(unweighted_mean = rowData(spe_unweighted)$mean,
                        weighted_mean = unlist(lapply(weighted_nnSVG_list, function (x) x[c('weighted_mean')])),
