...
cancer_genes <- c("LSAMP", "DTX3L", "CASC15", "SLC25A6", "CDK9", "MAP3K8", "GCH1", "PCYT2",
                  "TUFT1", "DDX39B")

df_Ovarian$cancer <- ifelse(rownames(df_Ovarian) %in% cancer_genes, TRUE, FALSE)

# plot ranks on either axis
Ovarian_ranks <- ggplot(df_Ovarian, aes(x = unweighted, y = weighted, color = cancer)) +
  geom_point(size = 0.5) +
  geom_point(data = filter(df_Ovarian, cancer == T), aes(x = unweighted, y = weighted, color = cancer), size = 2) +
  geom_text_repel(data = filter(df_Ovarian, cancer == T), 
                  aes(x = unweighted, y = weighted, label = rownames(filter(df_Ovarian, cancer == T))),
                  size = 5, max.overlaps = 20, color = "black") + 
...
cancer_genes <- c("DDX20", "ASXL2", "MGAT5", "ICOS", "TNFRSF21", "NUP43", "CDK13", "NOL6", "DAPK1", "TRIM21",
                  "FAM111B", "SPDYC", "ZBTB1", "PEAK1", "RCCD1", "FTO", "ULK2", "TRIM65", "DNMT3B",
                  "ATXN7", "E2F3", "MFHAS1", "SAAL1", "DDX10", "TLN2", "ITCH", "ZHX3")

df_Breast$cancer <- ifelse(rownames(df_Breast) %in% cancer_genes, TRUE, FALSE)

# plot ranks on either axis
Breast_ranks <- ggplot(df_Breast, aes(x = unweighted, y = weighted, color = cancer)) +
  geom_point(size = 0.5) +
  geom_point(data = filter(df_Breast, cancer == T), aes(x = unweighted, y = weighted, color = cancer), size = 2) +
  geom_text_repel(data = filter(df_Breast, cancer == T), 
                  aes(x = unweighted, y = weighted, label = rownames(filter(df_Breast, cancer == T))),
                  size = 5, max.overlaps = 20, color = "black") + 
...
ggsave(here("plots", "main", "test_Breast.png"),
       wrap_plots(
         Breast_ranks,
         nrow = 1,
         guides = "collect",
...
cancer_genes <- c("CASP8", "ATP11B", "E2F3", "EHMT2", "TREM1", "SMC2", "CD82", "BIRC2", "GABARAPL1",
                  "CDYL2", "MSI2", "CTDP1", "DDA1", "JAM2", "TAF1", "FGF13")

df_SubtypeBreast$cancer <- ifelse(rownames(df_SubtypeBreast) %in% cancer_genes, TRUE, FALSE)

# plot ranks on either axis
SubtypeBreast_ranks <- ggplot(df_SubtypeBreast, aes(x = unweighted, y = weighted, color = cancer)) +
  geom_point(size = 0.5) +
  geom_point(data = filter(df_SubtypeBreast, cancer == T), aes(x = unweighted, y = weighted, color = cancer), size = 2) +
  geom_text_repel(data = filter(df_SubtypeBreast, cancer == T), 
                  aes(x = unweighted, y = weighted, label = rownames(filter(df_SubtypeBreast, cancer == T))),
                  size = 5, max.overlaps = 20, color = "black") + 
  xlim(c(1, n_genes_SubtypeBreast)) +
  ylim(c(1, n_genes_SubtypeBreast)) +
  geom_abline(slope=1, intercept=0) +
  ggtitle("Subtype Breast") +
  theme_bw() +
  My_Theme

ggsave(here("plots", "main", "test_SubtypeBreast.png"),
       wrap_plots(
         SubtypeBreast_ranks,
         nrow = 1,
         guides = "collect",
...
cancer_genes <- c("ZNF436", "MTF2", "HAGLR", "SATB1", "CD200", "TRIM59", "UCHL1", "LIFR", "FER",
                  "NFIL3", "ENTPD1", "CLEC2D", "FLT1", "CGRRF1", "TYRO3", "ADAM10", "NLRP1", "HAUS5", "MECP2",
                  "PDK1", "PPAT", "ATP6AP1L", "LYRM7", "TRIM35", "TRIM32", "FRAT1", "TIGAR", "ZNF439", "MOSPD2", "BRWD3")

df_LobularBreast$cancer <- ifelse(rownames(df_LobularBreast) %in% cancer_genes, TRUE, FALSE)

# plot ranks on either axis
LobularBreast_ranks <- ggplot(df_LobularBreast, aes(x = unweighted, y = weighted, color = cancer)) +
  geom_point(size = 0.5) +
  geom_point(data = filter(df_LobularBreast, cancer == T), aes(x = unweighted, y = weighted, color = cancer), size = 2) +
  geom_text_repel(data = filter(df_LobularBreast, cancer == T), 
                  aes(x = unweighted, y = weighted, label = rownames(filter(df_LobularBreast, cancer == T))),
                  size = 5, max.overlaps = 20, color = "black") + 
