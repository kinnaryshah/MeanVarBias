---
title: "calculate_weights_breast"
...
r <- t(as.matrix(counts(spe_breast)))

n <- dim(spe_breast)[2] # Number of Cells
G <- dim(spe_breast)[1] # Number of Genes
...
coords <- spatialCoords(spe_breast)
...
sum(lambda_hat < range(r_tilda)[1] | lambda_hat > range(r_tilda)[2]) / (dim(spe_breast)[1]*dim(spe_breast)[2]) 
sum(lambda_hat < range(r_tilda)[1]) / (dim(spe_breast)[1]*dim(spe_breast)[2])
sum(lambda_hat > range(r_tilda)[2]) / (dim(spe_breast)[1]*dim(spe_breast)[2])
...
saveRDS(w, file = "breast_BRISC_estimation_spline.rds")

w <- readRDS(file = "mean_var_project/breast_estimation_spline.rds")
