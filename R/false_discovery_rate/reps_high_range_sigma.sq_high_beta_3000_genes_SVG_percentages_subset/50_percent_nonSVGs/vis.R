...
create_FDR_df <- function(spe_unweighted, spe_weighted) {
  ...
  adj_p_values_weighted <- p.adjust(1 - pchisq(rowData(spe_weighted)$weighted_LR_statdf=2), method = "BH")
...
weighted_ridge_combined_unw_mean <- df %>%
  ...
  labs(
    y = "quantile - unw mean of loounts",
    x = "rank",
...
df <- data.frame(
  ...
  mean = rowData(spe_unweighted)$mean
) %>% mutate(quantile = fdInterval(mean,
                                     quantile(mean, probs=0:9/10))) %>%
  tibble::rownames_to_column()
