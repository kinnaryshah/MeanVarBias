...
nb_model <- function(layer){
  ...
  mean_emp = rowMeans(counts(spe_layer))
  var_emp = genefilter::rowVars(counts(spe_layer))
  ...
  fit_nb =  glmGamPoi::glm_gp(counts(spe_layer), design = ~ 1, size_factors = FALSE, 
                          overdispersion = TRUE, on_disk = FALSE)

  mean_var_tb = tibble(mean_emp = mean_emp,
                       var_emp = var_emp,
                       nbinomial = mean_emp + mean_emp^2 * fit_nb$overdispersions) %>% 
...
spe_layer <- spe_151507[, colData(spe_151507)$spatialLIBD == "L2"]
ix_low_genes <- rowSums(counts(spe_layer)) < n_umis
spe_layer <- spe_layer[!ix_low_genes, ]
...
p = all_layers_df %>%
  ...
  geom_line(data = all_layers_df %>% filter(model %in% c("nbinomial")),
