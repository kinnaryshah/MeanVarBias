...
df_fxn <- function(layer) {
  ...
  df_effect$log_LR_stat <- log(df_effect$LR_stat + 1)
...
var <- ggplot(all_layers_df, 
       aes(x = mean, y = var, color = log_LR_stat)) + 
  geom_point(size = 2) + 
...
spat_var <- ggplot(all_layers_df, 
       aes(x = mean, y = sigma.sq, color = log_LR_stat)) + 
  geom_point(size = 2) + 
  facet_grid(layer_val~., switch = "y") +
  scale_color_viridis(trans = "log10") +
  scale_color_gradient(low = "blue", high = "red") +
...
nonspat_var <- ggplot(all_layers_df, 
       aes(x = mean, y = tau.sq, color = log_LR_stat)) + 
  geom_point(size = 2) + 
  facet_grid(layer_val~., switch = "y") +
  scale_color_viridis(trans = "log10") +
  scale_color_gradient(low = "blue", high = "red") +
...
prop <- ggplot(all_layers_df, 
       aes(x = mean, y = prop_sv, color = log_LR_stat)) + 
  geom_point(size = 2) + 
  facet_grid(layer_val~., switch = "y") +
  scale_color_viridis(trans = "log10") +
  scale_color_gradient(low = "blue", high = "red") +
  labs(x = "mean logcounts", 
       y = "", 
       color = "log LR stat",
       title = "proportion spatial variance") + 
  theme_bw() + 
  theme(strip.background = element_blank(), strip.text.y = element_blank())
